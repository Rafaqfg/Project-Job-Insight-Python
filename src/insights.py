...
def get_unique_job_types(path):
    ...
    """
    jobs = read(path)
    jobs_types = {job["job_type"] for job in jobs}
    return jobs_types
...
def get_min_salary(path):
    ...
    """
    jobs = read(path)
    jobs_salaries = {int(job["min_salary"])
                     for job in jobs
...
def matches_salary_range(job, salary):
    ...
    if (
        ...
        (
            float(job["max_salary"]).is_integer() is False |
            float(job["min_salary"]).is_integer() is False |
            float(salary).is_integer() is False
        ) or
        job["min_salary"] > job["max_salary"]
    ):
        raise ValueError
    if job["min_salary"] <= salary <= ["max_salary"]:
        return True
    return False
